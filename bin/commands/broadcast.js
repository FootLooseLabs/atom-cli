...
var broadcast = async (broadcastString) => {
  //broadcastString format = @flpl/devops:::GetIntro:::{}
  process.nucleus.on("ready", async () => {
    console.log("\n");
    var targetInterfaceName, targetInterfaceOp, payload;

    try {
      [targetInterfaceName, targetInterfaceOp, payload] =
        broadcastString.split(":::");
    } catch (e) {
      console.error(
        "ERROR: ",
        "couldnt parse broadcastString = ",
        broadcastString,
        ` ${e.message}`,
      );
      console.log("exiting...");
    ...
    if (!targetInterfaceName || !targetInterfaceOp) {
      console.error(
        "ERROR: ",
        "invalid targetInterfaceName or targetInterfaceOp = (",
        targetInterfaceName,
        ", ",
        targetInterfaceOp,
        ")",
    ...
    try {
      var signalStatus = await AtomSignal.publishToInterface(
        `${targetInterfaceName}:::${targetInterfaceOp}`,
