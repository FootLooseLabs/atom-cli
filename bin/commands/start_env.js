...
var startInterface = async (_interface) => {
  ...
  console.log(
    `INFO: Starting Interface ${_interface._name}`,
    "\n",
  ...
  if (getKwarg("mode") == "dev") {
    ...
    try {
      let _interfaceSubprocess = exec(
        `cd ${_interface.absDir} && sudo npm run start`,
      );
      ...
      console.debug(
        "DEBUG: ",
        `Started ${_interface._name} at dir = ${_interface.absDir}`,
      );
    } catch (e) {
      console.error("-------------:Error:------------- \n ", e);
    }
